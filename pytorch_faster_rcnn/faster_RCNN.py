...
class FasterRCNNLightning(pl.LightningModule):
    ...
    def validation_epoch_end(self, outs):
        ...
        #this part can go after everything works, don't need per class AP since only one class
        for key, value in per_class.items():
            self.log(f"Validation_AP_{key}", value["AP"])

    def test_step(self, batch, batch_idx):
        #start timer to record calculate FPS
        t0 = time.time()
        # Batch
        x, y, x_name, y_name = batch
        ...
        return {"pred_boxes": pred_boxes, "gt_boxes": gt_boxes, "step_time": step_time}
    ...
    def test_epoch_end(self, outs):
        ...
        pred_boxes = [out["pred_boxes"] for out in outs]
        pred_boxes = list(chain(*pred_boxes))
        step_time = [out["step_time"] for out in outs]

        #print("number of test images: " + str(sum(pred_boxes)))
        fps = 10/sum(step_time)
        ...
        per_class, m_ap = metric["per_class"], metric["m_ap"]
        self.log("Test_mAP", m_ap, "FPS", fps)
